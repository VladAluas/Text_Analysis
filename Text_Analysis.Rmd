...
<div class = "tags">
**Text Analysis**, **Sentiment Analysis**, **Automatization**
</div>
...
Recently I found myself with some free time on my hands so I decided to learn a new skill, or at least start learning. So I tought to myself, what would be a good skill that would help me as a data analyst or would have helped me in the past? It had to be something that took a lot of time and that could be automated.
  
And then it struck me, **TEXT ANALYSIS!** I often found myself, or colleagues of mine found themselves, in the annoying situation in which I had to read countless comments about a product, or read countless transcripts of focus groups, and try to figure out what is the message of the transcripts / comments, what is different from one text to another or one product to another and especially what is the sentiment transmitted by the text.
  
As you might know if you've done this sort of work, it can be a really boring and tedious work and it can take a lot of time. Especially if it is a recurring report, a lot of your time will be spent on reading and quantifying comments or focus group transcripts. I think of this as a waste of time for an analyst because the time can be better spent discussing the results, elaborating hypothesis on what can cause the effect and proposing solutions. 
  
So, in order to see what can be done for the above situation I have started reading  [**Text Mining with R**](https://www.tidytextmining.com/index.html) by *Julia Silge* and *David Robinson*. I highly recommend this book as their approach is to transform the text into a tidy format that allows you to easily analyse and visualize the results using graphs. As a plus, the process is easy to automatize.
...
This article is intended just as an introductory example into what text analysis can do and how it can be used by data analysts to free up a lot of their time. It is not intended as a comprehensive course on Natural Language Processing (NLP), as that is a complex topic that cannot be dealt with in just one article. Here, I will just show you three methods that can cover a great deal of analytical needs in a company, and they can free up a lot of your time as an data analyst.
  
I would also like to point out that I will show some basic sentiment analysis methods, however they do not cover all the possibilities and are just the tip of the iceberg. That being said, with some tweaking, they can reliably be used as a starting point in the endeavors to automate this process, with more complex methods being added at a latter time.
...
As a dataset, I though that as series of phone reviews would be a good starting point. 
...
```{r word_frequency_facet_wrap, warning=FALSE}
reviews_tidy %>%
  group_by(Model) %>% 
  word_frequency(5) +
...
```{r wordcloud_sentiment, warning=FALSE}
library(reshape2)
...
This is a vey quick and useful way to show which elements influence the sentiment for your product the most and make decisions based on it. 
...
This article is just a quick glance into text analysis and the capabilities of `R` to help you in your day to day tasks. 
  
When I read the book, I realised how much time my colleagues or I would spend on a task that can take a few hours to set up once and then just let the script run and do the task or you. This was really eye-opening to me and I became an even greater supporter of automatization.

Let's be honest, as a data analyst you will always have tasks that are repetitive and boring but have to be done. 

These can be something simple or very complex, however their accuracy most of the time depends on your ability to focus. We are all humans and have a bad day from time to time, and it's normal, so why let this influence the results of our analysis when you can automate the task?

Software, statistically speaking, has way less off days than humans do.
